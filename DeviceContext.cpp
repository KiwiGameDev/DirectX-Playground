...
void DeviceContext::setTextureVertexShader(const TexturePtr& texture)
{
...
{
	UINT stride = vertex_buffer->m_size_vertices;
	UINT offset = 0;

	m_device_context->IASetVertexBuffers(0, 1, &vertex_buffer->m_buffer, &stride, &offset);
	m_device_context->IASetInputLayout(vertex_buffer->m_layout.m_layout);
}
...
void DeviceContext::drawIndexedTriangleList(UINT index_count, UINT start_index_location, UINT start_vertex_location)
{
	m_device_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	m_device_context->DrawIndexed(index_count, start_index_location, start_vertex_location);
}
